...
async def generate_color_palette(
    ...
    try:
        ...
        print(f"🎯 Apply immediately: {request.applyImmediately}")
        print(f"📊 Existing colors: {len(request.existingColors)}")

        # Generate color palette using the service
        result = await color_palette_service.generate_color_palette(
            project_id=project_id,
            keywords=request.keywords,
            existing_colors=request.existingColors,
            apply_immediately=request.applyImmediately
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate color palette: {str(error)}"
        )
...
async def apply_color_palette(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to apply color palette: {str(error)}"
        )
...
async def get_existing_colors(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get existing colors: {str(error)}"
        )