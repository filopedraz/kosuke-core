...
async function saveUploadedFile(file: File, projectId: string): Promise<MessageAttachmentPayload> {
  ...
  try {
    ...
    console.log(`⬆️ File uploaded to storage: ${uploadResult.fileUrl}`);
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      if (attachmentPayload) {
        console.log(`✅ File uploaded: ${attachmentPayload.upload.fileUrl} (${attachmentPayload.upload.fileType})`);
      }
    } else {
