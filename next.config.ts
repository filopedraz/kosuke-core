...
const nextConfig: NextConfig = {
  ...
  // Security headers for local development (production uses Traefik)
  async headers() {
    ...
    const cspHeader = `
      default-src 'self';
      script-src 'self' ${isDev ? "'unsafe-eval' 'unsafe-inline'" : ''} https://*.clerk.accounts.dev https://challenges.cloudflare.com https://plausible.io https://*.cookiebot.com ${posthogHost};
      ...
      frame-src 'self' ${isDev ? 'http://localhost:* http://127.0.0.1:*' : ''} https://challenges.cloudflare.com https://*.clerk.accounts.dev;
      object-src 'none';
      base-uri 'self';
      form-action 'self';
