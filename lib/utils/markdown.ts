...
/**
 * Synchronous version of renderSafeMarkdown for backwards compatibility
 * Note: This version does not include Shiki syntax highlighting
 * @param content - The markdown content to render
 * @returns The rendered and sanitized HTML string
 */
export function renderSafeMarkdownSync(content: string): string {
  const html = renderMarkdown(content);
  ...
  // Style code blocks without syntax highlighting
  safeHtml = safeHtml
    .replace(
      /<pre><code/gi,
      '<pre class="bg-muted rounded-md p-3 overflow-x-auto w-full max-w-full my-3 text-left"><code class="text-xs whitespace-pre-wrap"'
    )
    .replace(
      /<pre><code\s+class="language-([^"]*)"([^>]*)>/gi,
      '<pre class="bg-muted rounded-md p-3 overflow-x-auto w-full max-w-full my-3 text-left"><code class="text-xs whitespace-pre-wrap language-$1"$2>'
    );

  // Apply all other styling (same as async version)
  safeHtml = safeHtml
    .replace(
      /<a\s+href="([^"]*)"([^>]*)>/gi,
      '<a href="$1" target="_blank" rel="noopener noreferrer"$2>'
